(() => {
  ...
  async function publicLaboratorySource(url, init) {
    ...
    try {
      ...
      const response = await originalFetch(`${LAB_RAW}${sourcePath}${separator}v=${Date.now()}`, {
        ...(init || {}),
        cache: 'no-store',
      });
  ...
  window.fetch = async function questLocalDataFetch(input, init) {
    ...
    // The laboratory repository is public again. Use it as the freshest source
    // for both explicit Pages URLs and Quest's committed mirror paths. The local
    // mirror remains a resilience fallback if GitHub raw content is unavailable.
